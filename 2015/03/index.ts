...
export default class Puzzle extends AoCPuzzle {
  public async part1(): Promise<string | number> {
    ...
    for (const char of chars) {
      switch (char) {
        case '^': coords[1] += 1; break;
        case 'v': coords[1] -= 1; break;
        case '<': coords[0] -= 1; break;
        case '>': coords[0] += 1; break;
  ...
  public async part2(): Promise<string | number> {
    ...
    for (const char of chars) {
      switch (char) {
        case '^': (i % 2 === 0 ? coords1 : coords2)[1] += 1; break;
        case 'v': (i % 2 === 0 ? coords1 : coords2)[1] -= 1; break;
        case '<': (i % 2 === 0 ? coords1 : coords2)[0] -= 1; break;
        case '>': (i % 2 === 0 ? coords1 : coords2)[0] += 1; break;
        default: throw new Error(`Invalid character: ${char}`);
      }
