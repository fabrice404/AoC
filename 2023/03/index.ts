...
export class Puzzle extends AoCPuzzle {
    ...
    this.lines.forEach((line, row) => {
      ...
      for (let i = 0; i < line.length; i += 1) {
        ...
        if (posStart !== -1 && posEnd !== -1) {
          ...
          let posLeft = Math.max(posStart - 1, 0);
          let posRight = Math.min(posEnd + 1, line.length - 1);
  ...
  public part2(): string | number {
    ...
    while (potentialGearParts.length > 0) {
      ...
      const indexFound = potentialGearParts.findIndex((potentialGearPart) => {
        return potentialGearPart.symbols.find((symbol) => {
          return symbol.col === gear?.col && symbol.row === gear?.row
        });
      });
      if (indexFound !== -1) {
